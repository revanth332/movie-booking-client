...
export default function MovieCard({movie}:{movie:Movie}) : JSX.Element {
  return (
    <Card className="h-[330px] bg-slate-200">
        ...
        <CardContent>
        <CardTitle>
        {movie.movie_name.length > 20 ? movie.movie_name.substring(0,20)+"..." : movie.movie_name}
        </CardTitle>
        <br/>
        <CardDescription>
          <div className="flex justify-between">
          <Button><Link to={movie.movie_id} >Watch</Link></Button>
          <p className="text-red-500 font-bold flex items-center"> Rating : {movie.rating}</p>
          </div>
